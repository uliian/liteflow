...
mvn -pl liteflow-testcase-el/liteflow-testcase-el-rule-db-etcd-springboot \
  -am clean verify -DskipTests=false -Dmaven.test.skip=false

reports="liteflow-testcase-el/liteflow-testcase-el-rule-db-etcd-springboot/target/surefire-reports"
require_passing_report "$reports/com.yomahub.liteflow.repository.etcd.EtcdWatchResilienceTest.txt"
require_passing_report "$reports/com.yomahub.liteflow.repository.etcd.EtcdContainerIntegrationTest.txt"
...
coverage_csv="liteflow-testcase-el/liteflow-testcase-el-rule-db-etcd-springboot/target/site/jacoco-aggregate/jacoco.csv"
test -f "$coverage_csv"
awk -F, '
  NR > 1 && $1 ~ /liteflow-rule-db-etcd$/ {
  ...
  END {
    ...
    printf "liteflow-rule-db-etcd coverage: lines %.2f%%, branches %.2f%%\n", line_rate, branch_rate;
...
if [[ "$java_specification_version" != "1.8" && "${java_specification_version%%.*}" -ge 17 ]]; then
  mvn -pl liteflow-testcase-el/liteflow-testcase-el-rule-db-etcd-springboot4 \
