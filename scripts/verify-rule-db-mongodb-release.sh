...
mvn -pl liteflow-testcase-el/liteflow-testcase-el-rule-db-mongodb-springboot \
  -am clean verify -DskipTests=false -Dmaven.test.skip=false

reports="liteflow-testcase-el/liteflow-testcase-el-rule-db-mongodb-springboot/target/surefire-reports"
require_passing_report "$reports/com.yomahub.liteflow.repository.mongodb.MongoPollingResilienceTest.txt"
require_passing_report "$reports/com.yomahub.liteflow.repository.mongodb.MongoTestcontainersIntegrationTest.txt"
...
coverage_csv="liteflow-testcase-el/liteflow-testcase-el-rule-db-mongodb-springboot/target/site/jacoco-aggregate/jacoco.csv"
test -f "$coverage_csv"
awk -F, '
  NR > 1 && $1 ~ /liteflow-rule-db-mongodb$/ {
  ...
  END {
    ...
    printf "liteflow-rule-db-mongodb coverage: lines %.2f%%, branches %.2f%%\n", line_rate, branch_rate;
...
springboot4_module="liteflow-testcase-el/liteflow-testcase-el-rule-db-mongodb-springboot4"
if [[ -d "$springboot4_module" ]] \
  && [[ "$java_specification_version" != "1.8" && "${java_specification_version%%.*}" -ge 17 ]]; then
  mvn -pl "$springboot4_module" \
    -am clean test -DskipTests=false -Dmaven.test.skip=false
else
  echo "skip springboot4 segment: $springboot4_module not present or JDK < 17"
fi
